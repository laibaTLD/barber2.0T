...
function resolveHeroMediaUrl(raw: unknown): string {
  if (!raw) return '';
...
export function getPrimaryHeroImageFromHero(hero?: Page['hero']): string {
  ...
  for (const raw of candidates) {
    const url = resolveHeroMediaUrl(raw);
    if (url) return url;
  }
  return '';
}

export type CmsImageRef = { url: string; altText?: string };

/** Normalize CMS image fields (string path, `{ url }`, or nested `image`). */
export function normalizeCmsImageRef(raw: unknown): CmsImageRef | undefined {
  ...
  if (typeof raw === 'object' && raw !== null) {
    ...
    if (record.image) {
      return normalizeCmsImageRef(record.image);
    }
  }
  return undefined;
}

/** About / overview sections may store images under several CMS keys. */
export function resolveSectionImageFromRecord(
  ...
  const candidates = [
    ...
    data.media,
    Array.isArray(data.mediaItems) ? data.mediaItems[0] : undefined,
    data.featuredImage,
  ];

  for (const raw of candidates) {
    const image = normalizeCmsImageRef(raw);
    if (image) return image;
  }
  return undefined;
}

/** Resolved image URL for a section, then optional fallback refs (e.g. hero, CTA). */
export function resolveSectionImageUrl(
  data: Record<string, unknown>,
  fallbacks: CmsImageRef[] = []
): string | undefined {
  const primary = resolveSectionImageFromRecord(data);
  if (primary) return getImageSrc(primary.url);
  for (const ref of fallbacks) {
    if (ref?.url) return getImageSrc(ref.url);
  }
  return undefined;
}
...
export function collectServiceAreaImageFallbacks(
  ...
  sitePages: Page[],
  order: Array<'hero' | 'cta' | 'about' | 'homeAbout' | 'homeCta'> = [
    'hero',
    'cta',
    'about',
    'homeAbout',
  ...
  const home = sitePages.find((p) => p.pageType === 'home');
  const byKey: Record<string, CmsImageRef | undefined> = {
    hero: page?.hero
      ? resolveSectionImageFromRecord(page.hero as Record<string, unknown>)
      : undefined,
    cta: page?.cta
      ? resolveSectionImageFromRecord(page.cta as Record<string, unknown>)
      : undefined,
    about: page?.about
      ? resolveSectionImageFromRecord(page.about as Record<string, unknown>)
      : undefined,
    homeAbout: home?.aboutSection?.image,
    homeCta: home?.ctaSection
      ? resolveSectionImageFromRecord(home.ctaSection as Record<string, unknown>)
      : undefined,
  };

  const refs: CmsImageRef[] = [];
